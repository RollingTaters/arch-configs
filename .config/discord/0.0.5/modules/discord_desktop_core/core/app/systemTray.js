...
function init(_options) {
  ...
  _ipcMain2.default.on('SYSTEM_TRAY_SET_ICON', function (evt, icon) {
    return setTrayIcon(icon);
  });
  _ipcMain2.default.on('SYSTEM_TRAY_SET_APPLICATIONS', function (evt, newApplications) {
    return setApplications(newApplications);
  });
}
...
function buildContextMenu() {
  ...
  var hasApplications = applications != null && applications.length > 0;

  contextMenu = [menuItems[MenuItems.SECRET], separator].concat(_toConsumableArray(hasApplications ? [].concat(_toConsumableArray(applications), [separator]) : []), [menuItems[MenuItems.OPEN], menuItems[MenuItems.MUTE], menuItems[MenuItems.DEAFEN], menuItems[MenuItems.VOICE_SETTINGS], menuItems[MenuItems.CHECK_UPDATE], menuItems[MenuItems.ACKNOWLEDGEMENTS], separator, menuItems[MenuItems.QUIT]]);
}
...
function setApplications(newApplications) {
  applications = newApplications.map(function (application) {
    return {
      type: 'normal',
      label: application.name,
      click: function click() {
        return launchApplication(application.id);
      }
    };
  });
  buildContextMenu();
